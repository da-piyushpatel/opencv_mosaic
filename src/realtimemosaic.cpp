...
void ImgCallback(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    try{
       framegrab.rgb = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::BGR8)->image;
...
int main(int argc, char** argv)
{
    ...
    image_transport::Subscriber image_sub = it.subscribe("/mv/image_raw",1,ImgCallback);
    ...
    while(ros::ok()){
        ...
        if(::count == 0)
        {
			...
			result = framegrab.rgb;
	        cv::cvtColor(result,result,COLOR_BGR2GRAY);

        }

        currframe = framegrab.rgb;
        // printf("2");
        if(currframe.empty())
    		break;
